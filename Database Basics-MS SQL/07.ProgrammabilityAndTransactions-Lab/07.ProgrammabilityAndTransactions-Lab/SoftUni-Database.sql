...
--Functions
CREATE OR ALTER FUNCTION udf_ProfectDurationWeeks(@StartDate DATETIME, @EndDate DATETIME)
RETURNS INT
AS
BEGIN
...
StartDate,
EndDate,
[dbo].udf_ProfectDurationWeeks(StartDate, EndDate) AS Weeks
--[SoftUni].[dbo].udf_ProfectDurationWeeks(StartDate, EndDate)
FROM Projects

--Procedures

--CREATE PROC
CREATE OR ALTER PROCEDURE dbo.usp_SelectEmployeesBySeniority
AS
	SELECT *
	FROM Employees
...
ALTER PROCEDURE dbo.usp_SelectEmployeesBySeniority
AS
...
--Check if any object depends on the stored procedure
EXEC sp_depends 'usp_SelectEmployeesBySeniority'

CREATE OR ALTER PROCEDURE usp_SelectEmployeesBySeniority(@minYearsAtWork int=5)
AS
	SELECT FirstName, LastName, HireDate,
		DATEDIFF(YEAR, Hiredate, GETDATE()) As Years
		FROM Employees
...
EXEC udp_AssignProject 1, 5 --Error

EXEC udp_AssignProject 2, 5 --Passes
...
UPDATE Towns SET [Name]='' WHERE TownId=1--trigger returns error
